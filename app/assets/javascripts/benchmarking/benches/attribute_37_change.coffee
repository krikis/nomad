...
Benches.setupAttribute37Change = (next) ->
  # define model
...
Benches.beforeAttribute37Change = (next) ->
  # create the original data object
  @answerOriginal = Util.randomObject()
  @answer = new @Answer _.deepClone @answerOriginal
  # perform the winning update
  [@dummyOriginal, deleted] = Util.randomVersion(@answerOriginal, change: 0.375, changeOdds: [0, 1, 0])
  @dummy = new @Answer _.deepClone @dummyOriginal
  # perform the losing update
  [version, deleted] = Util.randomVersion(@answerOriginal, change: 0.375, changeOdds: [0, 1, 0])
  @answer.set version
  _.each deleted, (property)=>
    @answer.unset property
  next.call @

Benches.attribute37Change = (next) ->
  try
