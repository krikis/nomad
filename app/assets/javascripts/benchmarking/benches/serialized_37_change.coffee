...
Benches.setupSerialized37Change = (next) ->
...
Benches.beforeSerialized37Change = (next) ->
  changeRate = 0.375
  # create the original data version
  @originalVersion = Util.randomObject()
  # handle local update
  [@localVersion, localDeleted] = Util.randomVersion(@originalVersion, change: changeRate, changeOdds: [0, 1, 0])
  ...
  [@remoteVersion, remoteDeleted] = Util.randomVersion(@originalVersion, change: changeRate, changeOdds: [0, 1, 0])
...
Benches.serialized37Change = (next) ->
  try
